...
class Creator(nc.Creator):
    ...
    def __init__(self):
        ...
        self.output_cutviewer_comments = True
    ...
    def tool_change(self, id):
        if self.output_comment_before_tool_change:
            self.comment('tool #' + str(self.tool_defn_params[id]['index']) + ' Z%.3f' % float(self.tool_defn_params[id]['tool length offset']) + ' "' + self.tool_defn_params[id]['name'] + '"');
        ...
        if self.output_h_and_d_at_tool_change == True:
            if self.output_g43_on_tool_change_line == False:
                self.write(self.SPACE() + 'G43')
            self.write(self.SPACE() + 'H' + str(id) + '\n')
#            self.write(self.SPACE() + 'D' + str(id) + self.SPACE() + 'H' + str(id) + '\n')
    ...
    def tool_defn(self, id, name='',params=None):
        ...
        self.tool_defn_params[id]['name'] = name
        self.tool_defn_params[id]['index'] = id
        if self.output_tool_definitions:
            if self.output_tool_definitions_commented:
                self.write( '(')
            ...
            if (params['tool length offset'] != None):
                self.write(self.SPACE() + 'Z%.3f' % float(params['tool length offset']))
